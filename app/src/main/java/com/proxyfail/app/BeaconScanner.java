...
public class BeaconScanner {
    ...
    public void stopScan() {
        ...
        if (callback != null && foundBeaconId == null) {
            // Classify common reasons for failure to give user-friendly feedback
            String errorMsg;
            if (bluetoothLeScanner == null) {
                errorMsg = "❌ Bluetooth LE not supported on this device.";
            } else {
                BluetoothManager bm = (BluetoothManager) context.getSystemService(Context.BLUETOOTH_SERVICE);
                if (bm == null) {
                    errorMsg = "❌ Bluetooth subsystem unavailable.";
                } else if (!bm.getAdapter().isEnabled()) {
                    errorMsg = "⚙️ Bluetooth is turned off. Please enable it and retry.";
                } else if (ContextCompat.checkSelfPermission(context,
                        Manifest.permission.BLUETOOTH_SCAN) != PackageManager.PERMISSION_GRANTED) {
                    errorMsg = "🚫 Missing permission: BLUETOOTH_SCAN.";
                } else if (ContextCompat.checkSelfPermission(context,
                        Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                    errorMsg = "📍 Missing permission: ACCESS_FINE_LOCATION.";
                } else {
                    errorMsg = "⏰ Scan timed out — no beacon signal detected nearby.";
                }
            }
        ...
        public void onScanFailed(int errorCode) {
            ...
            switch (errorCode) {
                ...
                case ScanCallback.SCAN_FAILED_APPLICATION_REGISTRATION_FAILED:
                    reason = "App registration with Bluetooth stack failed.";
                    break;
                case ScanCallback.SCAN_FAILED_FEATURE_UNSUPPORTED:
                    reason = "BLE scanning not supported on this device.";
                    break;
                case ScanCallback.SCAN_FAILED_INTERNAL_ERROR:
                    reason = "Internal Bluetooth error (possible firmware issue).";
                    break;
                default:
                    reason = "Unknown scan failure (code " + errorCode + ").";
            }
...
// }
// BluetoothAdapter bluetoothAdapter = bluetoothManager.getAdapter();
// bluetoothLeScanner = (bluetoothAdapter != null) ?
...
//
// boolean uuidMatches = (expectedServiceUuid == null); // keep existing
// behaviour
// String sessionIdFromBeacon = null;
//
...
// }
//
//// fallback: if no uuids or we still haven't found data, try the known
// PROXYFAIL_UUID directly
// if (!uuidMatches && expectedServiceUuid != null) {
// // try direct known UUID if expectedServiceUuid equals our constant
// if
// (expectedServiceUuid.equalsIgnoreCase(PROXYFAIL_UUID.getUuid().toString())) {
// byte[] fallback = record.getServiceData(PROXYFAIL_UUID);
// if (fallback != null && fallback.length > 0) {
// uuidMatches = true;
// sessionIdFromBeacon = new String(fallback, StandardCharsets.UTF_8);
